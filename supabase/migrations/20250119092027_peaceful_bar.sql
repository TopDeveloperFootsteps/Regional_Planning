...
CREATE TABLE care_setting_optimization_data (
    ...
    shift_flow_details text NOT NULL,  -- New column for shift flow details
...
-- Insert updated data with shift flow details
...
(care_setting, current_encounters, current_percentage, shift_potential, shift_direction, 
 potential_shift_percentage, proposed_percentage, potential_encounters_change, optimization_strategy, shift_flow_details)
SELECT 
    ...
    CASE 
        WHEN sc."care setting" = 'HOME' THEN 
            'Receiving setting: +' || FLOOR(100.0 * ts.total_shifted_encounters / (SELECT SUM(total_encounters) FROM encounter_stats)) || 
            '% total increase, receiving cases from: ' ||
            '- Health Station (15% of their volume), ' ||
            '- Ambulatory Service Center (25% of their volume), ' ||
            '- Specialty Care Center (10% of their volume), ' ||
            '- Extended Care Facility (5% of their volume), ' ||
            '- Hospital (5% of their volume)'
        WHEN sc."care setting" = 'HEALTH STATION' THEN 
            'Bidirectional flow: ' ||
            'Sending 15% of volume to Home care, ' ||
            'Receiving cases from higher acuity settings'
        WHEN sc."care setting" = 'AMBULATORY SERVICE CENTER' THEN 
            'Outward flow: -25% of current volume shifting to: ' ||
            '- Home care (primary destination), ' ||
            '- Health Station (secondary destination)'
        WHEN sc."care setting" = 'SPECIALTY CARE CENTER' THEN 
            'Outward flow: -10% of current volume shifting to: ' ||
            '- Home care, ' ||
            '- Health Station, ' ||
            '- Ambulatory Service Center'
        WHEN sc."care setting" = 'EXTENDED CARE FACILITY' THEN 
            'Outward flow: -5% of current volume shifting to: ' ||
            '- Home care (with support), ' ||
            '- Health Station (with monitoring)'
        WHEN sc."care setting" = 'HOSPITAL' THEN 
            'Outward flow: -5% of current volume shifting to: ' ||
            '- Home care (with enhanced support), ' ||
            '- Health Station (with monitoring), ' ||
            '- Ambulatory Service Center (for suitable cases)'
        ELSE 'No shift flow'
    END as shift_flow_details
