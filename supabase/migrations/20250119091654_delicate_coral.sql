...
SELECT 
    CASE e."care setting"
        ...
        ELSE e."care setting"
    END as care_setting,
    e.total_encounters as current_encounters,
    e.current_percentage as current_percentage,
    CASE 
    ...
    CASE 
        WHEN e."care setting" = 'HOME' THEN 0
        WHEN e."care setting" = 'HEALTH STATION' THEN 15
        WHEN e."care setting" = 'AMBULATORY SERVICE CENTER' THEN 25
        WHEN e."care setting" = 'SPECIALTY CARE CENTER' THEN 10
        WHEN e."care setting" = 'EXTENDED CARE FACILITY' THEN 5
        WHEN e."care setting" = 'HOSPITAL' THEN 5
        ELSE 0
    END as potential_shift_percentage,
    CASE 
        WHEN e."care setting" = 'HOME' THEN e.current_percentage + 10
        WHEN e."care setting" = 'HEALTH STATION' THEN e.current_percentage - 5
        ELSE e.current_percentage - CASE 
            WHEN e."care setting" = 'AMBULATORY SERVICE CENTER' THEN 25
            WHEN e."care setting" = 'SPECIALTY CARE CENTER' THEN 10
            WHEN e."care setting" = 'EXTENDED CARE FACILITY' THEN 5
            WHEN e."care setting" = 'HOSPITAL' THEN 5
    ...
    CASE 
        WHEN e."care setting" = 'HOME' THEN FLOOR(e.total_encounters * 0.1)
        WHEN e."care setting" = 'HEALTH STATION' THEN -FLOOR(e.total_encounters * 0.05)
        ELSE -FLOOR(e.total_encounters * CASE 
            WHEN e."care setting" = 'AMBULATORY SERVICE CENTER' THEN 0.25
            WHEN e."care setting" = 'SPECIALTY CARE CENTER' THEN 0.10
            WHEN e."care setting" = 'EXTENDED CARE FACILITY' THEN 0.05
            WHEN e."care setting" = 'HOSPITAL' THEN 0.05
            ELSE 0
        END)
    END as potential_encounters_change,
    CASE 
        WHEN e."care setting" = 'HOME' THEN 'Primary target for care shift - can receive cases from other settings through enhanced home care capabilities'
        WHEN e."care setting" = 'HEALTH STATION' THEN 'Optimize through bidirectional shift - transfer suitable cases to home care while receiving from higher acuity'
        WHEN e."care setting" = 'AMBULATORY SERVICE CENTER' THEN 'Focus on identifying cases suitable for lower acuity settings with proper support systems'
        WHEN e."care setting" = 'SPECIALTY CARE CENTER' THEN 'Implement specialist oversight programs for cases manageable in primary care settings'
        WHEN e."care setting" = 'EXTENDED CARE FACILITY' THEN 'Develop home care transition programs for suitable cases with adequate support'
        WHEN e."care setting" = 'HOSPITAL' THEN 'Create pathways for managing appropriate cases in lower acuity settings with proper monitoring'
