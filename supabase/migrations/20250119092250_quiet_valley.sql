...
-- Create new table with color coding for shifts
CREATE TABLE care_setting_optimization_data (
    ...
    incoming_shifts jsonb,  -- Store incoming shifts with percentages
    outgoing_shifts jsonb,  -- Store outgoing shifts with percentages
...
-- Insert updated data with detailed shift information
...
(care_setting, current_encounters, current_percentage, shift_potential, shift_direction, 
 ...
 incoming_shifts, outgoing_shifts)
SELECT 
    ...
    -- Incoming shifts (green)
    CASE 
        WHEN sc."care setting" = 'HOME' THEN 
            jsonb_build_object(
                'Health Station', 15,
                'Ambulatory Service Center', 25,
                'Specialty Care Center', 10,
                'Extended Care Facility', 5,
                'Hospital', 5
            )
        WHEN sc."care setting" = 'HEALTH STATION' THEN
            jsonb_build_object(
                'Ambulatory Service Center', 10,
                'Specialty Care Center', 5,
                'Extended Care Facility', 2,
                'Hospital', 2
            )
        ELSE NULL
    END as incoming_shifts,
    -- Outgoing shifts (red)
    CASE 
        WHEN sc."care setting" = 'HEALTH STATION' THEN
            jsonb_build_object('Home', 15)
        ...
        WHEN sc."care setting" = 'HOSPITAL' THEN
            jsonb_build_object(
                'Home', 2,
                'Health Station', 2,
                'Ambulatory Service Center', 1
            )
        ELSE NULL
    END as outgoing_shifts
