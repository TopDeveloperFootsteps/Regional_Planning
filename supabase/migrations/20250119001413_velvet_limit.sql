...
-- Create encounters_statistics view with whole number percentages
CREATE VIEW encounters_statistics AS
WITH total_encounters AS (
  SELECT SUM("number of encounters") as total
  FROM encounters
),
stats AS (
  SELECT 
...
SELECT 
  ...
  ROUND(total_encounter_count::numeric / t.total * 100) as encounter_percentage
FROM stats, total_encounters t
ORDER BY total_encounter_count DESC;

-- Create system_of_care_analysis view with whole number percentages
CREATE VIEW system_of_care_analysis AS
WITH care_setting_totals AS (
  SELECT 
    "system of care",
    "care setting",
    SUM("number of encounters") as setting_encounters,
    SUM(SUM("number of encounters")) OVER (PARTITION BY "system of care") as total_system_encounters
  FROM encounters
...
SELECT 
  ...
  SUM(e."number of encounters") as total_encounters,
  COUNT(DISTINCT e."icd family code") as unique_icd_codes,
  jsonb_object_agg(
    ...
    END,
    ROUND(cs.setting_encounters::numeric / cs.total_system_encounters * 100)
