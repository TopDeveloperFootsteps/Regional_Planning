-- Function to extract ICD code range from description
CREATE OR REPLACE FUNCTION extract_icd_range(description text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    RETURN replace(activity_str, ',', '')::numeric;
EXCEPTION
    WHEN OTHERS THEN
...
COMMENT ON COLUMN planning_code_sections.icd_sections IS 'Array of ICD code sections in format like Z00-Z13';
