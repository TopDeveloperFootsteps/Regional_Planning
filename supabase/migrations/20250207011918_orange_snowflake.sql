...
-- Add table constraint that enforces fixed values for specific population types
...
ALTER TABLE population_data ADD CONSTRAINT population_type_rules
    CHECK (
        CASE population_type
            WHEN 'Staff' THEN default_factor = 365 AND divisor = 365
            WHEN 'Residents' THEN default_factor = 365 AND divisor = 365
            WHEN 'Construction Worker' THEN default_factor = 365 AND divisor = 365
            WHEN 'Tourists/Visit' THEN divisor = 270 AND default_factor > 0
            WHEN 'Same day Visitor' THEN default_factor = 1 AND divisor = 365
            ELSE false
        END
    );