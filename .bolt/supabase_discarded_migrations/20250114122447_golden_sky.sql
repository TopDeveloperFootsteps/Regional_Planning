-- Update extended services mapping with specialized services from service list 2
...
SET 
    ...
    confidence = 'high',
    mapping_logic = CASE 
        WHEN LEFT(icd_code, 1) = 'A' OR LEFT(icd_code, 1) = 'B' THEN 'Infectious disease requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'C' THEN 'Cancer requiring extended facility treatment'
        WHEN LEFT(icd_code, 1) = 'D' THEN 'Blood disorder requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'E' THEN 'Endocrine disorder requiring extended facility management'
        WHEN LEFT(icd_code, 1) = 'F' THEN 'Mental health condition requiring extended psychiatric care'
        WHEN LEFT(icd_code, 1) = 'G' THEN 'Neurological condition requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'H' THEN 
            CASE 
            ...
            END
        WHEN LEFT(icd_code, 1) = 'I' THEN 'Cardiovascular condition requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'J' THEN 'Respiratory condition requiring extended pulmonology care'
        ...
        WHEN LEFT(icd_code, 1) = 'O' THEN 'Pregnancy/gynecological condition requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'P' THEN 'Pediatric condition requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'Q' THEN 'Congenital condition requiring extended genetic care'
        WHEN LEFT(icd_code, 1) = 'R' THEN 'General medical condition requiring extended facility care'
        WHEN LEFT(icd_code, 1) IN ('S', 'T') THEN 'Trauma/injury requiring extended emergency care'
        WHEN LEFT(icd_code, 1) IN ('V', 'W', 'X', 'Y') THEN 'Major emergency requiring extended facility care'
        WHEN LEFT(icd_code, 1) = 'Z' THEN 'Health factor requiring extended preventative care'
    END
...
SET 
    ...
    confidence = 'medium',
    mapping_logic = CASE 
        WHEN systems_of_care = 'Unplanned care' THEN 'Mapped based on unplanned care requirement in extended facility'
        WHEN systems_of_care = 'Planned care' THEN 'Mapped based on planned care requirement in extended facility'
        WHEN systems_of_care = 'Children and young people' THEN 'Mapped based on pediatric care requirement in extended facility'
        WHEN systems_of_care = 'Complex, multi-morbid' THEN 'Mapped based on complex care requirement in extended facility'
        WHEN systems_of_care = 'Chronic conditions' THEN 'Mapped based on chronic care requirement in extended facility'
        WHEN systems_of_care = 'Palliative care and support' THEN 'Mapped based on palliative care requirement in extended facility'
        WHEN systems_of_care = 'Wellness and longevity' THEN 'Mapped based on wellness requirement in extended facility'
    END
WHERE service = 'Pending Review';

-- Create a view to show mapping statistics by extended care service
