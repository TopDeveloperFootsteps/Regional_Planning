...
export function SystemOfCareDistribution({ data }: SystemDistributionProps) {
  const totalEncounters = data.reduce(
    (sum, item) => sum + Number(item.total_encounters),
    0
  );
          ...
          <div key={item.system_of_care} className="space-y-2">
            ...
            <div className="bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${(item.total_encounters / totalEncounters) * 100}%`,
                }}
              />
            </div>
            <div className="grid grid-cols-2 gap-2 mt-2">
              {Object.entries(item.care_setting_percentages).map(
                ([setting, percentage]) => (
