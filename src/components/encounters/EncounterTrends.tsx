...
export function EncounterTrends({ data }: EncounterTrendsProps) {
  const totalEncounters = data.reduce(
    (sum, item) => sum + Number(item.encounter_count),
    0
  );

  // Sort data by encounter count in descending order
  const sortedData = [...data].sort(
    (a, b) => b.encounter_count - a.encounter_count
  );
              ...
              <div className="flex justify-between items-baseline">
                ...
                <div className="text-right">
                  <span className="text-sm text-gray-900">
                    {item.encounter_count.toLocaleString()}
                  </span>
                  <span className="text-xs text-gray-500 ml-2">
                    (
                    {((item.encounter_count / totalEncounters) * 100).toFixed(
                      1
                    )}
                    %)
                  </span>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex-grow bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${
                        (item.encounter_count / totalEncounters) * 100
                      }%`,
                    }}
                  />
                </div>
              ...
              {data[0].top_icd_codes.map((code) => (
                <div key={code.icd_family_code} className="space-y-2">
                  <div className="flex justify-between items-baseline">
                    ...
                    <div className="text-right">
                      <span className="text-sm text-gray-900">
                        {code.encounters.toLocaleString()}
                      </span>
                      <span className="text-xs text-gray-500 ml-2">
