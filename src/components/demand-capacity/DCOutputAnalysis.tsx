...
export function DCOutputAnalysis() {
  ...
  const [debugInfo, setDebugInfo] = useState<{headers: string[]; separator: string}>();
  ...
  const handleCapacityFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          setCapacityError(err instanceof Error ? err.message : 'Error processing file');
        }
      };
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          setActivityError(err instanceof Error ? err.message : 'Error processing file');
        }
      };
      reader.readAsText(file);
    }
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPlanDetails(prev => ({ ...prev, population: parseInt(e.target.value) || 0 }))}
                  className="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPlanDetails(prev => ({ ...prev, date: e.target.value }))}
                  className="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                />
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 mb-2">Your file must have these exact column headers:</p>
              <code className="block bg-gray-100 p-3 rounded text-sm font-mono text-gray-800">
                Care Type,Reason for Visit / Specialty,Capacity Type,Inperson,Virtual
              </code>
          ...
          {debugInfo && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <p>Detected separator: {debugInfo.separator === ',' ? 'Comma (CSV)' : 'Tab (TSV)'}</p>
                <p>Detected headers:</p>
                <ul className="list-disc list-inside pl-4 space-y-1">
                  {debugInfo.headers.map((header, i) => (
                    <li key={i} className="font-mono">{header}</li>
                  ))}
                </ul>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Detailed Capacity Requirements</h2>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center justify-center w-full">
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-gray-500">CSV or TSV file with required columns</p>
                  </div>
                  <input 
                    type="file" 
                    className="hidden" 
              ...
              {capacityError && (
                <p className="mt-2 text-sm text-red-600 whitespace-pre-line">{capacityError}</p>
              )}
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Detailed Activity/Demand</h2>
            ...
            <div className="mb-6">
              ...
              {activityError && (
                <p className="mt-2 text-sm text-red-600 whitespace-pre-line">{activityError}</p>
              )}
            </div>
          ...
          {(capacityData.length > 0 || activityData.length > 0) && (
            <div className="flex flex-col items-end space-y-2">
              ...
              <button
                onClick={handleSave}
                disabled={isSaving || !planDetails.name || planDetails.population <= 0}
