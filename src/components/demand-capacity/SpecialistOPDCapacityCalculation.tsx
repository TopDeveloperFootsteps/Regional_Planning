...
export function SpecialistOPDCapacityCalculation() {
  ...
  const calculateAndSaveSlots = async (
    ...
    try {
      ...
      //     "http://localhost:5000/api/save_capacity_calculations",
      //     calculations
      //   );
  ...
  const fetchData = async () => {
    try {
      ...
      const [daysData, hoursData, timesData] = await Promise.all([
        api.get("/opdCapacity/available_days_s"),
        api.get("/opdCapacity/working_hours_s"),
        api.get("/opdCapacity/visit_times_s"),
      ]);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-6">
          Specialist OPD Slot Calculations
        </h2>
