...
export function DemandAnalysis() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [popDataResponse, regionsResponse, ratesData, genderData] =
        await Promise.all([
          api.get("/output/population_data"),
          api.get("/output/regions"),
          api.get("/output/visit_rates"),
          api.get("/output/gender_distribution"),
        ]);
      ...
      setPopulationData(popDataResponse || []);
      // Fetch visit rates

      const transformedRates: Record<
        string,
        Record<string, Record<string, { male: number; female: number }>>
      > = {};
  ...
  const getGenderDistribution = (ageGroup: string, year: number) => {
    const ageGroupData = genderDistribution.find(
  ...
  const calculateServiceVisits = () => {
    ...
    const relevantPopulation = populationData.filter(
      (pd) => selectedRegion === "all" || pd.region_id === selectedRegion
    );
    ...
    details += `\nTotal Population (${selectedYear}): ${Math.round(
      totalPopulation
    )}\n`;
      ...
      if (pd.population_type === "Staff") {
        ...
        AGE_GROUPS.forEach((ageGroup) => {
          if (workingAgeGroups.includes(ageGroup)) {
            const ageGroupPopulation =
              calculatedValue *
              workingAgeDistribution[
                ageGroup as keyof typeof workingAgeDistribution
              ];
            const { male: maleRatio, female: femaleRatio } =
              getGenderDistribution(ageGroup, selectedYear);
            ...
            details += `  Male: ${Math.round(malePopulation)} (${(
              maleRatio * 100
            ).toFixed(1)}%)\n`;
            details += `  Female: ${Math.round(femalePopulation)} (${(
              femaleRatio * 100
            ).toFixed(1)}%)\n`;
            ...
            SERVICES.forEach((service, index) => {
              const rates =
                visitRates[service]?.[ageGroup]?.[selectedAssumption];
              if (!rates) {
                console.warn(
                  `No rates found for ${service} - ${ageGroup} - ${selectedAssumption}`
                );
                return;
              }
              ...
              details += `    Male Rate: ${
                rates.male
              }/1000, Visits: ${Math.round(maleVisits)}\n`;
              details += `    Female Rate: ${
                rates.female
              }/1000, Visits: ${Math.round(femaleVisits)}\n`;
            });
          } else {
        ...
        AGE_GROUPS.forEach((ageGroup) => {
          ...
          SERVICES.forEach((service, index) => {
            ...
            details += `  ${service}:\n`;
            details += `    Male Rate: ${rates.male}/1000, Visits: ${Math.round(
              maleVisits
            )}\n`;
            details += `    Female Rate: ${
              rates.female
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedAssumption(
                    e.target.value as "model" | "enhanced" | "high_risk"
                  )
                }
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
              >
