...
function OPDRoomRequirements() {
  ...
  const calculateRoomRequirements = (
    ...
    visitRates.forEach((rate) => {
      ...
      serviceVisits.set(
        rate.service,
        serviceVisits.get(rate.service)! + totalVisits
      );
    });
    ...
    serviceVisits.forEach((visits, service) => {
      const specialtyRate = specialtyRates.find(
        (sr) => sr.specialty === service
      );
  ...
  const fetchData = async () => {
    try {
      ...
      const [
        ...
        api.get("/opdRoom/regions"),
        api.get(`/opdRoom/visit_rates?assumption_type=${selectedAssumption}`),
        api.get(`/opdRoom/population_data?region_id=${selectedRegion}`),
        api.get("/opdRoom/specialty_occupancy_rates"),
        api.get("/opdRoom/working_hours"),
        api.get("/opdRoom/available_days"),
      ]);
